class ChivalryCombatGame {
    ...
    updatePlayerMovement() {
        ...
        this.gameState.playerPos.x = Math.max(50, Math.min(this.canvas.width - 50, this.gameState.playerPos.x));
        this.gameState.playerPos.y = Math.max(50, Math.min(this.canvas.height - 50, this.gameState.playerPos.y));
    ...
    drawStartScreen() {
        ...
        this.ctx.fillText('CHIVALRY 2', this.canvas.width / 2, 200);
        
        this.ctx.fillStyle = '#F7931E';
        this.ctx.font = 'bold 32px Arial';
        this.ctx.shadowBlur = 10;
        this.ctx.fillText('COMBAT TRAINER', this.canvas.width / 2, 250);
        
        // Draw subtitle
        this.ctx.fillStyle = '#FFFFFF';
        this.ctx.font = 'bold 24px Arial';
        this.ctx.shadowBlur = 5;
        this.ctx.fillText('Master the Art of Medieval Combat', this.canvas.width / 2, 300);
    ...
    drawPalmTrees() {
        const treePositions = [
            ...
            { x: 350, y: this.canvas.height * 0.75 },
            { x: 850, y: this.canvas.height * 0.72 },
            { x: 1050, y: this.canvas.height * 0.78 }
        ];
    ...
    drawRocks() {
        const rockPositions = [
            { x: 100, y: this.canvas.height * 0.8 },
            { x: 250, y: this.canvas.height * 0.85 },
            { x: 900, y: this.canvas.height * 0.82 },
            { x: 1100, y: this.canvas.height * 0.88 }
        ];
